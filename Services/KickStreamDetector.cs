...
{
    public class KickStreamDetector : BaseStreamDetector
    {
        public KickStreamDetector(ILogger logger, IExecutableLocator executableLocator)
        ...
        {
            return !string.IsNullOrEmpty(url) && url.Contains("kick.com");
        }
        ...
        public override async Task<StreamInfo> GetStreamInfoAsync(string url)
        {
            try
            {
                _logger.LogInformation($"Récupération des informations du stream Kick pour : {url}");
                ...
                return new StreamInfo
                {
                    ChannelName = url.Split('/').Last(),
                    StreamUrl = url,
                    Title = "Stream Kick en direct",
                    IsLive = true
                };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Erreur lors de la récupération des informations du stream Kick pour {url}");
                return new StreamInfo
                {
                    ChannelName = url.Split('/').Last(),
                    StreamUrl = url,
