...
{
    public class SeventeenLiveStreamDetector : BaseStreamDetector
    {
        public SeventeenLiveStreamDetector(ILogger logger, IExecutableLocator executableLocator)
        ...
        {
            return !string.IsNullOrEmpty(url) && url.Contains("17.live");
        }
        ...
        public override async Task<StreamInfo> GetStreamInfoAsync(string url)
        {
            try
            {
                _logger.LogInformation($"Récupération des informations du stream Seventeen Live pour : {url}");
                ...
                return new StreamInfo
                {
                    ChannelName = url.Split('/').Last(),
                    StreamUrl = url,
                    Title = "Stream Seventeen Live en direct",
                    IsLive = true
                };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Erreur lors de la récupération des informations du stream Seventeen Live pour {url}");
                return new StreamInfo
                {
                    ChannelName = url.Split('/').Last(),
                    StreamUrl = url,
