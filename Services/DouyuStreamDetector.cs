...
{
    public class DouyuStreamDetector : BaseStreamDetector
    {
        public DouyuStreamDetector(ILogger logger, IExecutableLocator executableLocator)
        ...
        {
            return !string.IsNullOrEmpty(url) && url.Contains("douyu.com");
        }
        ...
        public override async Task<StreamInfo> GetStreamInfoAsync(string url)
        {
            try
            {
                _logger.LogInformation($"Récupération des informations du stream Douyu pour : {url}");
                ...
                return new StreamInfo
                {
                    ChannelName = url.Split('/').Last(),
                    StreamUrl = url,
                    Title = "Stream Douyu en direct",
                    IsLive = true
                };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Erreur lors de la récupération des informations du stream Douyu pour {url}");
                return new StreamInfo
                {
                    ChannelName = url.Split('/').Last(),
                    StreamUrl = url,
