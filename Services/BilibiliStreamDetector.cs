...
namespace Strivea.Services
{
    ...
    {
        public BilibiliStreamDetector(ILogger logger, IExecutableLocator executableLocator)
        ...
        {
            return !string.IsNullOrEmpty(url) && url.Contains("bilibili.com");
        }
        ...
        public override async Task<StreamInfo> GetStreamInfoAsync(string url)
        {
            try
            {
                _logger.LogInformation($"Récupération des informations du stream Bilibili pour : {url}");
                ...
                return new StreamInfo
                {
                    ChannelName = url.Split('/').Last(),
                    StreamUrl = url,
                    Title = "Stream Bilibili en direct",
                    IsLive = true
                };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Erreur lors de la récupération des informations du stream Bilibili pour {url}");
                return new StreamInfo
                {
                    ChannelName = url.Split('/').Last(),
                    StreamUrl = url,
